...
bool MOQOutput::ResolveServiceConfig()
{
    ...
    if (key && *key) 
    {
        blog(LOG_INFO, "[obs-moq] using Namespace from service: %s", key); // TODO: erase this log line after testing
        stream_key = key;
    }
    else
    {
...
bool MOQOutput::CreateVideoTrack(moq_media_sender_t *new_sender)
{
	...
	tcfg.packaging = MOQ_MEDIA_PACKAGING_RAW; // todo: make this configurable and add CMAF support
	tcfg.codec = {(const uint8_t *)codec_string.c_str(), codec_string.size()};
	tcfg.timescale = VIDEO_TIMESCALE;
	tcfg.init_data = {init_data.data(), init_data.size()}; // todo: analyze actual need for this and how it fits w/other codecs
	tcfg.is_live = true;
...
bool MOQOutput::Connect()
{
	...
	ecfg.insecure_skip_verify = true; //todo: dynamically resolve from config
...
void MOQOutput::Data(struct encoder_packet *packet)
{
    ...
    uint64_t pts_usec = 0;
    pts_usec = util_mul_div64((uint64_t)packet->pts,
                                1000000ull * (uint64_t)packet->timebase_num,
    ...
    moq_result_t res;
    {
        ...
        if (!sender || !video_track) {
			moq_rcbuf_decref(payload); // release the rcbuf since we won't be sending it
			return;
		}
...
void register_moq_output()
{
    ...
    info.id = "moq_output";
    info.flags = OBS_OUTPUT_VIDEO | OBS_OUTPUT_ENCODED | OBS_OUTPUT_SERVICE; // todo: change to OBS_OUTPUT_AV when audio is supported 
    info.protocols = "MOQ";
    info.encoded_video_codecs = "h264"; // todo: add support for hevc and av1
    // info.encoded_audio_codecs = "aac;opus"; // todo: add support for audio
